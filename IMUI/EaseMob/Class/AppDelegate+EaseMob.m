...
#define APP_KEY @"58city#58sherlock"


#define APNS_CER_NAME @"sherlock"
...
- (void)easemobApplication:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (![self isSpecifyServer]) {
        [[EaseMob sharedInstance] registerSDKWithAppKey:APP_KEY
                                           apnsCertName:APNS_CER_NAME
                                            otherConfig:@{kSDKConfigEnableConsoleLogger:@YES}];
//        SLUser * user = [SLUserHistory resumeUserHistory].currentUser;
//        if (user && ![user.userId isEqualToString:EMPTY_STRING]) {
//            [[EaseMob sharedInstance].chatManager asyncLoginWithUsername:user.userId password:kIMLoginPassword];
//        }
    }
...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken{
    ...
    NSString * token = [[deviceToken debugDescription] stringByTrimmingCharactersInSet:[NSCharacterSet characterSetWithCharactersInString:@"<>"]];
}
...
- (void)didBindDeviceWithError:(EMError *)error
{
    if (error) {
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:nil
                                                        message:NSLocalizedString(@"apns.failToBindDeviceToken", @"Fail to bind device token")
                                                       delegate:nil
