...
#define kTopViewHeight 52

@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, EMCDDeviceManagerDelegate,EMCallManagerDelegate,MWPhotoBrowserDelegate>
{
...
@property (strong , nonatomic) NSMutableArray* assets;//照片库的图片
...
//    [[HYPhotoBrowserAssetsHelper sharedInstance] loadAssetsWithBlock:^(NSMutableArray<ALAsset *> *assets) {
//        self.assets = assets;
//    }];
...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, kTopViewHeight, self.view.frame.size.width, self.view.frame.size.height - kTopViewHeight - self.chatToolBar.frame.size.height) style:UITableViewStylePlain];
...
- (void)chatVideoCellPressed:(MessageModel *)model
{
    ...
    __weak ChatViewController *weakSelf = self;
    id <IChatManager> chatManager = [[EaseMob sharedInstance] chatManager];
    [weakSelf showHudInView:weakSelf.view hint:@"视频下载中"];
...
-(void)chatImageCellBubblePressed:(MessageModel *)model
{
    ...
    if ([model.messageBody messageBodyType] == eMessageBodyType_Image) {
        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            ...
            [weakSelf showHudInView:weakSelf.view hint:@"图片下载中..."];
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                [weakSelf hideHud];
                if (!error) {
                    //发送已读回执
            ...
            for (int i = 0; i < weakSelf.dataSource.count; i ++) {
                ...
                if ([object isKindOfClass:[MessageModel class]]) {
                    ...
                    if ([message.messageId isEqualToString:model.messageId]) {
                        MessageModel *cellModel = [MessageModelManager modelWithMessage:message context:self.context];
        ...
        for (int i = 0; i < self.dataSource.count; i ++) {
            ...
            if ([object isKindOfClass:[MessageModel class]]) {
                ...
                if ([messageId isEqualToString:currMsg.messageId]) {
                    ...
                    MessageModel *cellModel = [MessageModelManager modelWithMessage:currMsg context:self.context];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [weakSelf.tableView beginUpdates];
                        [weakSelf.dataSource replaceObjectAtIndex:i withObject:cellModel];
                        [weakSelf.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:i inSection:0]] withRowAnimation:UITableViewRowAnimationNone];
                    ...
                    if (error && error.errorCode == EMErrorMessageContainSensitiveWords)
                    {
                        ...
                        [self showHint:@"您发送的消息包含敏感词" yOffset:-frame.size.height + KHintAdjustY];
                    }
                    break;
                }
            }
...
//    [self presentViewController:self.imagePicker animated:YES completion:NULL];
    ...
    _photos = [HYPhotoBrowserAssetsHelper photosFromAssets:self.assets];
    _thumbs = [HYPhotoBrowserAssetsHelper thumbsFromAssets:self.assets];
...
- (void)didFinishRecoingVoiceAction:(UIView *)recordView
{
    ...
    [[EMCDDeviceManager sharedInstance] asyncStopRecordingWithCompletion:^(NSString *recordPath, NSInteger aDuration, NSError *error) {
        if (!error) {
            ...
            hud.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"record_failed"]];
...
- (NSArray *)formatMessages:(NSArray *)messagesArray
{
    ...
    if ([messagesArray count] > 0) {
        for (EMMessage *message in messagesArray) {
            ...
            MessageModel *model = [MessageModelManager modelWithMessage:message context:self.context];
...
- (void)photoBrowserDidFinishModalPresentation:(MWPhotoBrowser *)photoBrowser {
    ...
    [hud showAnimated:YES whileExecutingBlock:^{
        ...
        for (NSInteger index = 0; index<[_selections count]; index++) {
            ...
            [HYPhotoBrowserAssetsHelper imageFromAsset:self.assets[value] complete:^(UIImage *image) {
...
#pragma mark - SLChatTopViewDelegate
//- (void)contractButtonDidClicked:(UIButton *)sender {
//    SLOrderViewController * orderViewController =  [[SLOrderViewController alloc] initWithContext:nil];
