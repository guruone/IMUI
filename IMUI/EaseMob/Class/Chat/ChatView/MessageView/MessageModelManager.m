...
+ (id)modelWithMessage:(EMMessage *)message context:(HYContext *)context
{
...
//        model.username = message.from;
        model.username = context.contextDic[@"userNick"];
    }

    if (isSender) {
//        model.headImageURL = [NSURL URLWithString:[SLUserHistory resumeUserHistory].currentUser.userIcon] ;
    }
    else{
        model.headImageURL = [NSURL URLWithString:context.contextDic[kChatUserHead]];
    }
